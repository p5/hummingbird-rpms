...
%global prevmajorversion15 15
%global prevversion15 %{prevmajorversion15}.10
%global prev_prefix15 %{_libdir}/pgsql/postgresql-%{prevmajorversion15}
...
Source18: https://ftp.postgresql.org/pub/source/v%{prevversion15}/postgresql-%{prevversion15}.tar.bz2
Source19: https://ftp.postgresql.org/pub/source/v%{prevversion15}/postgresql-%{prevversion15}.tar.bz2.sha256
...
Provides: bundled(postgresql-server) = %prevversion15
...
find postgresql-%{prevversion15} -type f -name Makefile -exec sed -i -e "s/SO_MAJOR_VERSION=\s\?\([0-9]\+\)/SO_MAJOR_VERSION= %{private_soname}-\1/" {} \;
%endif
...
%endif
	...
	pushd postgresql-%{prevversion15}
	...
	# The set of built server modules here should ideally create superset
	# of modules we used to ship in %%prevversion15 (in the installation
	# the user will upgrade from), including *-contrib or *-pl*
	# subpackages.  This increases chances that the upgrade from
	# %%prevversion15 will work smoothly.
...
%if %upgrade
	...
	rm lib/*.a
	# Drop libpq.  This might need some tweaks once there's
	# soname bump between %%prevversion15 and %%version.
	rm lib/libpq.so*
	# Drop libraries.
	rm lib/lib{ecpg,ecpg_compat,pgtypes}.so*
	rm share/*.bki
...
%%postgresql_upgrade_prefix16 %prev_prefix15
EOF
%endif
...
%changelog
* Sat Mar 29 2026 Factory <factory@p5.github.com> - 17.4-1
- Initial postgresql17 package
- Version 17.4
